...
def new_question(): #enter new question and 3 answer to DB
    ...
    db.insert({'number':it(),'questions':question,'answer1':ans1,'answer2':ans2,'answer3':ans3})
...
def find_answer():
    ...
    if ((db.search(find['questions'] == question)) and ((db.search(find['answer1'] == answer)) or (db.search(find['answer2'] == answer)) or (db.search(find['answer3'] == answer)))):
        return True
    return False
result=find_answer()
print(result)
